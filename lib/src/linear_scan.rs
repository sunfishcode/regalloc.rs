...
fn split_and_spill<F: Function>(
  ...
  //state.intervals.set_reg(child, state.intervals.location(id).reg().unwrap());
  //state.spill(child);
  ...
  //let child_start = state.intervals.start(child, &state.fragments);
